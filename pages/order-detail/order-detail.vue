		...
		<view class="goods-info">
			...
			<view v-for="goods in order.detail" class="goods" :key="goods.id">
				...
				<view class="goods-attr">
					<text v-for="attr in goods.goods_info.attr_list" :key="item.attr_id">
	...
	methods: {
		...
		toSendType() {
			...
			if (that.sendType == 1) {
				...
				if (that.status == 2) {
					uni.navigateTo({
						url: '/pages/send/send?order_refund_id=' + this.detail.id + '&mch_id=' + this.mch_id
					});
				} else {
					uni.navigateTo({
						url: '/pages/send/send?id=' + this.detail.id + '&mch_id=' + this.mch_id
					});
				}
			}
		},
