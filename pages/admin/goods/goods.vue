	...
	<app-layout>
		...
		<view class="search main-between">
			...
			<view v-else class="dir-left-nowrap">
				<input
					focus
					@blur="beSearch"
					class="box-grow-1 t-small search-content"
					@confirm="reload"
					confirm-type="search"
					v-model="keyword"
				/>
		...
		<view>
			<view class="msg-item" @click="id = null" v-for="item in list" :key="item.id">
				...
				<image class="more-handle" @click.stop="showMenu(item.id)" src="/static/image/myshop/more-handle.png"></image>
				<view v-if="item.id == id" class="main-between more">
					<view v-if="item.status == 1" @click.stop="toSwitch(0, item.id)">
						<view class="more-item">
							<image src="/static/image/myshop/goods-xiajia.png"></image>
							<view>下架</view>
						</view>
					</view>
					<view v-if="item.status == 0 && is_goods_audit == 0" @click.stop="toSwitch(1, item.id)">
						<view class="more-item">
							<image src="/static/image/myshop/goods-xiajia.png" style="transform: rotate(180deg)"></image>
							<view>上架</view>
						</view>
					</view>
					<view
						v-if="(item.mch_goods.status == 0 || item.mch_goods.status == 3) && item.status == 0 && is_goods_audit == 1"
						@click.stop="toSwitch(2, item.id)"
					>
						<view class="more-item">
							<image src="/static/image/myshop/goods-xiajia.png" style="transform: rotate(180deg)"></image>
							<view>申请上架</view>
					...
					<view @click.stop="toDelete(item.id)">
						<view class="more-item">
							<image src="/static/image/myshop/goods-del.png"></image>
							<view>删除</view>
						</view>
					</view>
				</view>
...
export default {
	data() {
		return {
			tabList: [{ id: 1, name: '出售中' }, { id: 2, name: '告罄' }, { id: 3, name: '仓库中' }, { id: 4, name: '最新' }],
			activeTab: '1',
			keyword: '',
			page: 1,
			id: null,
	...
	methods: {
		toAdd() {
			uni.navigateTo({
				url: '/pages/admin/add-goods/add-goods?mch_id=' + this.mch_id
			});
		},
		beSearch() {
