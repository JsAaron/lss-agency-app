	...
	<app-layout>
		<view>
			<view class="input-list">
				...
				<view class="dir-left-nowrap input-item" v-if="express == '京东物流' || express == '京东快运'">
		...
		<view class="dialog" v-if="dialog">
			<view class="picker-list">
				...
				<picker-view indicator-style="height: 36px;" class="picker" :value="[index]" @change="bindChange">
					<picker-view-column>
						<view v-for="item in list" :key="item.name" class="pick-view">{{ item.name }}</view>
					</picker-view-column>
				</picker-view>
			</view>
		</view>
		...
		getList() {
			...
			that
				...
				.then(response => {
					that.$hideLoading();
					if (response.code == 0) {
						that.express = response.data.detail.express;
						that.express_no = response.data.detail.express_no;
						that.customer_name = response.data.detail.customer_name;
						that.code = '';
						that.words = response.data.detail.words;
						if (response.data.detail.code > 0) {
							that.code = response.data.detail.code;
						}
					} else {
						uni.showToast({
							title: response.msg,
