			...
			<scroll-view scroll-x :scroll-into-view="`mch_` + scrollId" scroll-with-animation>
				<view class="dir-left-nowrap month-list">
					...
					<block v-for="(item, index) in year_list" :key="index">
						<block v-for="(year_item, year_index) in item.month_list" :key="year_index">
							<view
								@click="switchTaps(index, year_index)"
								class="box-grow-0 dir-top-nowrap main-center month-item"
								:class="year_item.active ? `active` : ``"
								:id="`mch_` + index + year_index"
							>
...
export default {
	...
	methods: {
			...
			self
				...
				.then(info => {
					...
					if (info.code === 0) {
						...
						[self.year_list, self.scrollId, self.year_str, self.month_str] = [year_list, scrollId, year_str, month_str];
						this.loadData();
					}
				});
		},
			...
			self
				...
				.then(info => {
					if (info.code === 0) {
						...
						[self.contrast_prev_monthly, self.monthly_order_pay_price_average, self.monthly_order_pay_price_count] = [
		...
		switchTaps(index, year_index) {
			...
			[self.index, self.year_list, self.year_index, self.scrollId, self.year_str, self.month_str] = [
				index,
				year_list,
				year_index,
				scrollId,
