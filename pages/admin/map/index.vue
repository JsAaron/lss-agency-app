	...
	<view class="container">
		<view class="header bg-ff">
			<view class="row padding border-b font-26">
				<view class="col ellipsis-1">
					<view class="ellipsis-1">
						<view class="">
							<text class="color">[当前]</text>
							{{ address.title }}
						</view>
						<view class="color-99 ellipsis-1">{{ address.address }}</view>
					</view>
				</view>
			...
			<view class="row padding font-26">
				<input class="col" placeholder="补充详细地址：门牌号、楼房、房间号" v-model="detail" />
		...
		>
			<cover-image :src="coverUrl" class="icon-img"></cover-image>
		</map>
		...
		<view class="footer bg-ff font-26">
			<scroll-view scroll-y class="scroll" :scroll-top="scrollTop">
				<view class="">
					<view
						class="padding border-b position-r"
...
var qqmapsdk = new QQMapWX({
	key: 'U5DBZ-THYRU-4UYVR-BNKLD-6ADI6-IKB4Y'
});

function getFileLocalPath(fileName, isPlatform) {
	if (isPlatform == 'android') {
		return 'file:/' + plus.io.convertLocalFileSystemURL('_www' + fileName);
	} else {
		return '/static/map/icon_position.png';
	}
}
...
export default {
	data() {
		return {
			...
			map: {
				...
				latitude: '',
				showLocation: true,
				iconPath: '/static/map/icon_position.png',
				width: 40,
				height: 40,
				scale: 16,
				controls: [
	...
	created() {
		...
		this.coverUrl = getFileLocalPath('/static/map/icon_position.png', this.isPlatform);
	},
	...
	onReady() {
		wx.getSystemInfo({
			success: response => {
				this.mapHeight = response.windowHeight - 310 + 'px';
			}
		});
		if (this.options.longitude && this.options.latitude) {
			this.setPosition(this.options.longitude, this.options.latitude);
		} else {
			this.getAddress();
		}
	},
	...
	methods: {
		getAddress() {
			uni.getLocation({
				type: 'gcj02',
				success: res => {
					this.setPosition(res.longitude, res.latitude);
				}
			});
		},
		...
		setPosition(longitude, latitude) {
			...
			this.getWidthHeight(e => {
				...
				if (this.isPlatform == 'android') {
					map.controls[0].position.top = e.height / 2 - 50;
		...
		getWidthHeight(fn) {
			...
			query.select('.mapelement').boundingClientRect();
			query.exec(res => {
				fn(res[0]);
			});
		},
