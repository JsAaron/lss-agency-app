	...
	<app-layout>
		...
		<view class="header-h">
			...
			<view class="header-box dir-top-nowrap main-center">
				...
				<app-form-id @click="navCash"><view class="cash-btn">提现</view></app-form-id>
			</view>
		</view>
		...
		<view class="footer"><view @click="showDesc">交易手续费说明</view></view>
	</app-layout>
</template>
...
export default {
	...
	methods: {
		navCash() {
			uni.navigateTo({
				url: `/pages/admin/cash/cash?mch_id=` + this.mch_id + `&account_money=` + this.account_money
			});
		},
		navAccountLog() {
			uni.navigateTo({ url: `/pages/admin/account-log/account-log?mch_id=` + this.mch_id });
		},
		navCashLog() {
			uni.navigateTo({ url: `/pages/admin/cash-log/cash-log?mch_id=` + this.mch_id });
		},
		navSettleDetailN() {
			uni.navigateTo({ url: `/pages/admin/settle-detail/settle-detail?is_transfer=0&mch_id=` + this.mch_id });
		},
		navSettleDetailY() {
			uni.navigateTo({ url: `/pages/admin/settle-detail/settle-detail?is_transfer=1&mch_id=` + this.mch_id });
		},
		loadData: function() {
			const self = this;
			...
			self
				...
				.then(info => {
					self.$hideLoading();
					[self.account_money, self.close_money, self.desc, self.not_close_money, self.transfer_fate] = [
