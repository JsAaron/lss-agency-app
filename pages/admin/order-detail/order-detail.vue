<template>
	<app-layout>
		...
		<view v-else class="head-txt main-between">
			...
			<view v-if="order.auto_confirm > 0 && order.is_send == 1 && order.is_confirm == 0">
				剩余
				<text v-if="dd > 0">{{ dd }}天</text>
				<text v-if="hh > 0 && dd == 0">{{ hh }}小时</text>
				{{ mm }}分自动确认收货
			</view>
			<view v-else-if="order.auto_cancel > 0 && order.is_pay == 0 && order.pay_type != 2">
				剩余
				<text v-if="dd > 0">{{ dd }}天</text>
				<text v-if="hh > 0 && dd == 0">{{ hh }}小时</text>
				{{ mm }}分自动删除
			</view>
			<view v-else-if="order.auto_sales > 0 && order.is_sale == 0 && order.is_confirm == 1">
				剩余
				<text v-if="dd > 0">{{ dd }}天</text>
				<text v-if="hh > 0 && dd == 0">{{ hh }}小时</text>
				{{ mm }}分自动完成订单
		...
		<view class="user-info">
			<image class="localhost-img" src="/static/image/myshop/localhost.png"></image>
			<view class="user-text">
				<text class="name">{{ status == 1 ? order.name : order.order.name }}</text>
				<text>{{ status == 1 ? order.mobile : order.order.mobile }}</text>
			</view>
			<view class="user-text address-info">{{ status == 1 ? order.address : order.order.address }}</view>
		...
		<view class="express" v-if="detail.express">
			<image class="logo" src="/static/image/myshop/express.png"></image>
			<view class="express-company">{{ detail.express }}</view>
			<view class="express-number">运单编号：{{ detail.express_no }}</view>
		</view>
		<view class="express" v-else-if="order.express">
			<image class="logo" src="/static/image/myshop/express.png"></image>
		...
		<view class="goods-info">
			...
			<view v-for="goods in order.detail" class="goods" :key="goods.id">
				...
				<view class="goods-attr">
					<text v-for="attr in goods.goods_info.attr_list" :key="attr.attr_name">
		...
		<view class="goods-info mch-goods">
			...
			<view
				v-if="order.order_form != '[]'"
				v-for="item in order.order_form"
				:key="item"
				class="dir-left-nowrap price-item"
			>
				<view class="price-label">{{ item.label }}:</view>
				<view v-if="item.key == 'img_upload'">
					<image @click="look(item.value)" class="form-img" :src="item.value"></image>
				</view>
		...
		<view
			:class="['dir-right-nowrap', 'btn', `${iphone_x ? 'iphone_x' : ''}`]"
			v-if="order.is_sale != 1 || status == 2"
		>
			...
			<view
				@click="toChange(order)"
				v-if="order.cancel_status == 0 && order.is_pay == 0 && order.is_send == 0"
			...
			<view
				@click="toSend(order)"
				v-if="order.cancel_status == 0 && order.is_send == 0 && order.is_pay == 1"
				class="handle-btn other"
			>
				发货
			</view>
			<view
				@click="toSend(order)"
				v-else-if="order.cancel_status == 0 && order.is_send == 0 && order.pay_type == 2"
				class="handle-btn other"
			>
				发货
			</view>
			<view
				open-type="navigate"
				@click="toExpress(order, 2)"
				v-if="order.cancel_status == 0 && order.is_send == 1 && order.is_confirm == 0 && order.express && status != 2"
				class="handle-btn other"
			>
				修改物流信息
			</view>
			<view @click="toCancelorder(order)" v-if="order.is_send == 0 && order.cancel_status == 0" class="handle-btn">
				取消订单
			</view>
			<view
				@click.stop="toRefund(order)"
				v-if="order.status_text == '已发货,待商家处理' && order.type == 1"
				class="handle-btn other"
			>
				退款
			</view>
			<view
				@click="toSend(order)"
				v-if="order.status_text == '已发货,待商家处理' && order.type == 2"
				class="handle-btn other"
			>
				发货
			</view>
			<view
				@click.stop="refundHandle(2, order, 1)"
				v-if="status == 2 && order.status == 2 && order.status_text != '已换货' && order.status_text != '已退款'"
				:class="['handle-btn', `${order.status_text == '已发货,待商家处理' ? '' : 'other'}`]"
			>
				取消售后
			</view>
		...
		>
			...
			<view class="dialog" v-if="changePrice">
				...
				<view class="dir-left-nowrap change-price-item">
					...
					<input type="digit" class="price-input" @input="priceInput" focus v-model="price" />
				</view>
				<view class="dir-left-nowrap change-price-item">
					<view class="label">运费</view>
					<view class="money">￥</view>
					<input type="digit" class="price-input" @input="expressInput" focus v-model="express" />
				</view>
				<view class="dir-left-nowrap change-price-item">
	...
	methods: {
		...
		toSendType() {
			...
			if (that.sendType == 1) {
				...
				if (that.status == 2) {
					uni.navigateTo({
						url: '/pages/admin/send/send?order_refund_id=' + this.detail.id + '&mch_id=' + this.mch_id
					});
				} else {
					uni.navigateTo({
						url: '/pages/admin/send/send?id=' + this.detail.id + '&mch_id=' + this.mch_id
					});
				}
			}
		},
		toExpress(e, is_send) {
			...
			if (id > 0) {
				uni.navigateTo({
					url: '/pages/admin/send/send?id=' + id + '&is_send=' + is_send + '&mch_id=' + this.mch_id
				});
			} else if (order_refund_id) {
				uni.navigateTo({
					url:
						'/pages/admin/send/send?order_refund_id=' +
						order_refund_id +
						'&is_send=' +
						is_send +
						'&mch_id=' +
