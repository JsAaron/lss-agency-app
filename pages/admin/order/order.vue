	...
	<app-layout>
		...
		<view v-if="!searchResult && search" :class="['search-history', `${list.length == 0 ? '' : 'go-out'}`]">
			<view v-if="candidate.length != 0" class="main-between search-title">
				<view>历史搜索</view>
				<view @click="clear"><image class="delete-icon" src="/static/image/icon/delete.png"></image></view>
			</view>
		...
		<view v-if="!search" class="search-area main-between cross-center">
			...
			<view class="choose" @click="toTime">
				<text v-if="date_start == ''">汇总</text>
				<text v-else-if="date_start == today && date_end == today">今日</text>
				<text v-else-if="date_start == yesterday && date_end == yesterday">昨日</text>
				<text v-else-if="date_start == weekday && date_end == today">7日</text>
				<text v-else>自定义</text>
				<image src="/static/image/myshop/open.png"></image>
			</view>
		</view>
		<view v-if="!search && status == 1" class="main-between navs-info">
			...
			<view @click="show = !show" v-if="active" class="box-grow-1 main-center navs more-navs">
				...
				<image src="/static/image/myshop/more-active.png"></image>
			</view>
			<view @click="show = !show" v-if="!active" class="box-grow-1 main-center navs more-navs">
				<text class="nav-text">更多状态</text>
				<image src="/static/image/myshop/more.png"></image>
			</view>
		</view>
		...
		<view>
			<view class="order-item" v-for="item in list" :key="item.id">
				<view @click="toDetail(item, status)">
					...
					<view class="dir-left-nowrap cross-center order-user">
						...
						<view @click.stop="toCall(item)" class="to-tel">
							<image src="/static/image/myshop/tel.png"></image>
							<text>联系收货人</text>
						</view>
					</view>
					<view v-for="goods in item.detail" class="goods" :key="goods.id">
						...
						<view class="goods-attr">
							<text v-for="attr in goods.goods_info.attr_list" :key="attr.attr_id">
				...
				<view class="dir-right-nowrap menu-button">
					...
					<view
						...
						v-if="item.cancel_status == 0 && item.is_pay == 0 && item.is_send == 0"
						class="handle-btn other"
					>
						修改价格
					</view>
					<view
						@click.stop="toExpress(item, 2)"
						v-if="
							item.have_refund != 1 &&
					...
					<view
						@click.stop="toSend(item)"
						v-if="item.cancel_status == 0 && item.is_send == 0 && item.is_pay == 1"
						class="handle-btn other"
					>
						发货
					</view>
					<view
						@click.stop="toSend(item)"
						v-else-if="item.cancel_status == 0 && item.is_send == 0 && item.pay_type == 2"
						class="handle-btn other"
					>
						发货
					</view>
					<view
						@click.stop="toRefundOrder(item.order_no)"
						v-if="item.have_refund == 1 && status == 1 && item.is_sale == 0"
						class="handle-btn"
					>
						售后中
					</view>
					<view
						@click.stop="toCancelorder(item)"
						v-if="item.is_send == 0 && item.cancel_status == 0"
						class="handle-btn"
					>
						取消订单
					</view>
					<view
						...
						v-if="
							item.status == 2 && item.status_text == '待退款' && item.is_confirm == 1 && status == 2 && item.type == 1
						"
						class="handle-btn other"
					>
						退款
					...
					<view
						@click.stop="toConfirm(item)"
						v-if="item.status == 2 && item.is_send == 1 && item.is_confirm == 0 && status == 2 && item.type == 1"
						class="handle-btn other"
					>
						确认收货
					</view>
					<view
						@click.stop="toSend(item)"
						data-send="1"
						v-if="item.is_confirm == 0 && status == 2 && item.is_send == 1 && item.type == 2"
						class="handle-btn other"
					>
						发货
					</view>
					<view
						@click.stop="refundHandle(2, item, 1)"
						v-if="item.status == 2 && status == 2 && item.is_refund == 0"
						:class="['handle-btn', `${_num == 2 || _num == -1 ? 'other' : ''}`]"
					>
						取消售后
					</view>
					<view @click.stop="refundHandle(1, item)" v-if="item.status == 1 && status == 2" class="handle-btn other">
						同意
					</view>
					<view @click.stop="refundHandle(2, item)" v-if="item.status == 1 && status == 2" class="handle-btn">
						拒绝
					</view>
		...
		>
			<!-- 选时间 -->
			<view class="dialog" v-if="chooseTime">
				<view class="dialog-title">筛选时间</view>
				<view class="flex-wrap main-between time-area">
					<view @click="change(0)" data-time="0" :class="[`${time == 0 ? 'active' : ''}`, 'dialog-choose-item']">
						汇总
					</view>
					<view @click="change(1)" data-time="1" :class="[`${time == 1 ? 'active' : ''}`, 'dialog-choose-item']">
						今日
					</view>
					<view @click="change(2)" data-time="2" :class="[`${time == 2 ? 'active' : ''}`, 'dialog-choose-item']">
						昨日
					</view>
					<view @click="change(3)" data-time="3" :class="[`${time == 3 ? 'active' : ''}`, 'dialog-choose-item']">
						7日
					</view>
					<view @click="change(4)" data-time="4" :class="[`${time == 4 ? 'active' : ''}`, 'dialog-choose-item']">
						自定义
					</view>
				...
				<view class="choose-time" v-if="custom">
					...
					<picker-view :value="start" class="picker-view" @change="startChange">
						...
						<picker-view-column>
							<view v-for="item in days" :key="item">{{ item }}</view>
						</picker-view-column>
					</picker-view>
					<view class="time-title">结束时间时间</view>
					<picker-view
						:value="end"
						indicator-style="height: 36px;color: #ff4544;font-size:14px;"
						class="picker-view"
						@change="endChange"
					>
						<picker-view-column>
			...
			<view class="dialog" v-if="noAddress">
				...
				<view class="main-center btn-area"><view class="submit-btn be-submit" @click="cancel">确认</view></view>
			</view>
			<!-- 打电话  -->
			...
			<view class="dialog" v-if="isReason">
				...
				<view class="reason">
					...
					<view class="dir-left-wrap reason-img" v-if="detail.pic_list.length > 0">
						<image @click="look(item)" v-for="item in detail.pic_list" :src="item"></image>
					</view>
				</view>
			...
			<view class="dialog" v-if="openAddress">
				...
				<view class="add-list">
					<view class="add-item" v-for="item in address" :key="item.id">
						<image
							v-if="item.id == addressId"
							@click="chooseAddress(item.id)"
							src="/static/image/myshop/apply-ok.png"
						></image>
						<image v-else @click="chooseAddress(item.id)" src="/static/image/myshop/apply-er.png"></image>
...
export default {
	...
	methods: {
		toDetail(item, status) {
			...
			uni.navigateTo({
				url: '/pages/admin/order-detail/order-detail?id=' + id + '&status=' + status + '&mch_id=' + this.mch_id
			});
		},
		toRedirect(url) {
			uni.redirectTo({
		...
		toSearch() {
			...
			this.candidate = uni.getStorageSync('mch_keyword');
			this.inSearch = false;
		},
		keywordSearch(e) {
			this.keyword = e;
		...
		// 搜索
		searchMethod() {
			var value = uni.getStorageSync('mch_keyword');
		...
		toExpress(e, is_send) {
			...
			if (id > 0) {
				uni.navigateTo({
					url: '/pages/send/send?id=' + id + '&is_send=' + is_send + '&mch_id=' + this.mch_id
				});
			} else if (order_refund_id) {
				uni.navigateTo({
					url:
						'/pages/send/send?order_refund_id=' +
						order_refund_id +
						'&is_send=' +
						is_send +
						'&mch_id=' +
		...
		toSendType() {
			...
			if (that.sendType == 1) {
				...
				if (that.detail.status_text) {
					uni.navigateTo({
						url: '/pages/send/send?order_refund_id=' + that.detail.id + '&mch_id=' + that.mch_id
					});
				} else {
					uni.navigateTo({
						url: '/pages/send/send?id=' + that.detail.id + '&mch_id=' + that.mch_id
					});
				}
			}
		},
