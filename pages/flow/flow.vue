			...
			<view v-else class="account-list">
				<view v-for="(item, index) in list" :key="index">
					<view class="account-item item flow-flex">
						<view class="member-image" v-if="item.user_id > 0">
							<image src="/static/image/image/member-icon.png"></image>
						</view>
						<view class="flex-item flow-left">
							<view class="flow-img" v-if="item.pay_way == 1">
								<image src="/static/image/myshop/flow-wx.png"></image>
							</view>
							<view class="flow-img" v-else-if="item.pay_way == 2">
								<image src="/static/image/myshop/flow-zfb.png"></image>
							</view>
							<view class="flow-img" v-else-if="item.pay_way == 3">
								<image src="/static/image/myshop/flow-yl.png"></image>
							</view>
							<view class="flow-img" v-else><image src="/static/image/myshop/flow-not.png"></image></view>
						</view>
						<view class="flex-item flow-bd">
							<view class="bd-name">订单金额：{{ numUtils(item.total_amount * 0.01) }}元</view>
							<view class="bd-name">结算金额：{{ numUtils(item.sj_real_amount * 0.01) }}元</view>
							<view class="bd-num">{{ item.order_no }}</view>
							<view class="bd-time">{{ item.created_at }}</view>
						</view>
