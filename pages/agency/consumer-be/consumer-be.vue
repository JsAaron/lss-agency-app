		...
		<view v-else class="list" >
		    <view class="item" v-for="item in list" :key="item.id">
				<view @click="toDetail(item.id)">
					...
					<view style="margin-bottom: #{8rpx};">交易时间：{{item.created_at}}</view>
					<view>商家名称：{{item.name}}</view>
					<view class="cash">
						<view class="cash-price" v-if="item.income_price == null || item.income_price == 0">0.00元</view>
						<view class="cash-price" v-else>{{mathnum((item.income_price)*0.01)}}元</view>
					</view>
				</view>
		    </view>
		</view>
...
<script>
    export default {
        ...
        methods: {
			...
			toDetail(id) {
			    uni.navigateTo({
			        url: '/pages/agency/consumer-be/details?id=' + id
			    });
			},
        },
