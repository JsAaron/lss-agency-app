		...
		<view class="info">
			...
			<view class="last-info">
				<view v-if="config.cash_max_day > -1">
					<view v-if="config.surplusCash > 0">
						今日剩余{{ custom_setting.words.cash_money.name }}
						<span class="surplus">{{ config.surplusCash ? config.surplusCash : config.cash_max_day }}元</span>
					</view>
				...
				<view @click="visible = !visible">
					<button :class="[`show-tip`, `${theme}-border`, `${theme}-color`]">规则</button>
				</view>
			</view>
		</view>
		<view>
			...
			<view class="about">
				<view>*提现平台将代缴个税，按6%计算，另每笔提现增加1元费用。</view>
				<view>*暂仅支持微信提现。</view>
			</view>

			<view class="dir-left-nowrap cross-center cash-type" @click="cashTypeModel = false">
				<view class="box-grow-1">{{ custom_setting.words.cash_type.name }}</view>
				<view class="box-grow-0">微信零钱</view>
			...
			<view class="cash-info" v-if="mark_num === 1 || mark_num === 2 || mark_num === 3">
				...
				<view class="info content">
					...
					<input
						...
						type="number"
						placeholder="请输入正确的银行卡帐号"
					/>
				</view>
			</view>
			<view class="submit"><button @click="subscribe" :class="[`${theme}-background`]">提交申请</button></view>
		</view>
		<app-cash-model
		...
		<view class="bg" v-if="visible">
			<view class="dialog">
				...
				<view class="dialog-msg">
					今日剩余{{ custom_setting.words.cash_money.name }}=平台每日可{{
						custom_setting.words.cash_money.name
					}}-今日所有用户{{ custom_setting.words.cash_money.name }}
				</view>
				<view class="dialog-btn main-center"><view class="dialog-submit" @click="visible = false">我知道了</view></view>
			</view>
		</view>
	</app-layout>
</template>
...
export default {
	...
	methods: {
		...
		submit() {
			...
			that
				.$request({
					url: 'https://lff.facess.net/web/index.php?_mall_id=1&r=api/agent/cash-add',
					data: para,
					method: 'post'
				})
				.then(response => {
