	...
	<view :style="{ 'z-index': zIndex }" @touchmove.stop="voidFc()" @tap.prevent.stop="voidFc()">
		...
		>
			<view class="flex_column" :class="(mode || 'middle') + '_view'" :style="{ height: heightSize }">
				<view
					v-if="mode === 'bottom'"
					class="flex_row_between_c width100 padding20unit padding_20_40unit"
					:style="{ 'font-size': titleSize, 'background-color': bgColor_title || '#f8f8f8' }"
				>
					<view
						class="flex_row_none_c width250unit "
						:style="{ color: buttonSet.cancleColor || '#ADADAD' }"
						@tap="hide()"
					>
						{{ buttonSet.cancleName || '取消' }}
					</view>
					<view class="flex_row_c_c width250unit" :style="{ color: titleColor || '#999' }">{{ title || '' }}</view>
					<view
						class="flex_row_e_c width250unit"
						:style="{ color: buttonSet.confirmColor || '#3399FF' }"
						@tap="confirm()"
					>
				...
				<view 
					v-if="mode === 'middle' && title"
					class="flex_row_c_c width100 padding20unit"
					:style="{
						'font-size': titleSize,
						'background-color': bgColor_title || '#f8f8f8',
				...
				<view
					v-if="mode === 'top'"
					class="flex_row_between_c width100 padding20unit"
					:style="{ 'font-size': titleSize, 'background-color': bgColor_title || '#f8f8f8' }"
				>
					<view
						class="flex_row_none_c width250unit "
			...
			<view class="flex_row" style="margin-top: 20px;" v-if="mode === 'middle'">
				<view class="flex_row_c_c width50">
					<button
						:type="buttonSet.cancelType || 'default'"
						@tap="hide()"
						:style="classObj.btnSize + (buttonSet.cancelStyle || '')"
					>
						{{ buttonSet.cancelName || '取消' }}
					</button>
				</view>
				<view class="flex_row_c_c width50">
					<button
						:type="buttonSet.confirmType || 'primary'"
						@tap="confirm()"
						:style="classObj.btnSize + (buttonSet.confirmStyle || '')"
					>
						{{ buttonSet.confirmName || '确定' }}
					</button>
				</view>
...
export default {
	...
	data() {
		const defaultSize = this.mode === 'middle' ? 0.3 : 0.45;
		...
		const defaultFontScale = this.fontScale || 0.034;
		// console.log('defaultFontScale:' + defaultFontScale);
		const titleSizeScale = Number(defaultFontScale) + 0.003;
		...
		return {
			showPicker: false,
			classObj: {
				btnSize: 'font-size: ' + wW * (this.fontScale + 0.005) + 'px;'
			},
			heightSize,
			titleSize,
			contentSize
