...
export default {
	...
	computed: {
		...
		getItem3() {
			if (this.async) {
				...
				return (
					(this.itemObject.step_3
						? this.itemObject.step_3[this.value[0] || 0]
							? this.itemObject.step_3[this.value[0] || 0][this.value[1] || 0]
							: []
						: []) || []
				);
			}
	...
	methods: {
		init() {
			if (Array.isArray(this.dataSet.defaultValue) && this.dataSet.defaultValue)
		...
		checkAsync() {
			if (this.async && this.itemObject.step_1 && this.itemObject.step_1.length > 0) {
				if (this.linkageNum == 2) {
					if (this.async_1.length !== 0) return;
				} else if (this.linkageNum == 3) {
					if (this.async_2 !== 0) return;
				}
				this.getAsyncData({
					value: this.value,
					launch: true
		...
		confirm() {
			...
			if (_this.linkage) {
				...
				if (_this.linkageNum == 2) {
					//二级联动
					data.steps1 = datas.step_1[v[0]];
					data.steps2 = async ? this.async_1[v[1]] : datas.step_2[v[0]][v[1]];
				} else if (_this.linkageNum == 3) {
					//三级联动
					data.steps1 = datas.step_1[v[0]];
					if (!data.steps1) _app.showToast('第一列中不存在第' + v[0] + '项');
					data.steps2 = async ? this.async_1[v[1]] : datas.step_2[v[0]][v[1]];
					if (!data.steps2) _app.showToast('第二列中不存在第' + v[1] + '项');
					data.steps3 = async ? this.async_2[v[2]] : datas.step_3[v[0]][v[1]][v[2]];
					if (!data.steps3) _app.showToast('第三列中不存在第' + v[2] + '项');
				} else {
