...
const _app = {
	...
	getUpLoadPromiseArray({
		...
		if (itemArray.length === 0 && required) {
			return Promise.reject({ ...errObj['102'],
				errMsg: '上传图片长度为零'
			});
		} else {
			const promiseArr = [];
...
function renderUrlData(u, d) {
	...
	if (d) {
		for (let i in d) {
			let itemd = d[i];
			if (itemd instanceof Object || Array.isArray(itemd)) {
			...
			for (const o in obj) {
				...
				switch (type) {
					case typeObj.input: //input
						...
						if (verifyType && verifyTypeObj[verifyType] !== undefined && verifyTypeObj[verifyType].reg && !verifyTypeObj[
								verifyType].reg.test(value)) {
							if (verifyItem.result) verifyItem.result = false;
					...
					case typeObj.pics: //pics
						if (isArrayAndHasLength(value)) {
							...
							if (notPathArr.length > 0) {
								...
								returnObj.data[o] = await (await _app.getUpLoadPromiseArray({
									itemArray: value,
									customId,
									upLoadData
								}));
