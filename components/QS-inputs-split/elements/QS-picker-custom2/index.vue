			...
			<view class="flex_row width100">
				<block v-if="linkage">
					<block v-if="linkageNum == 2">
						<view
							class="flex_row_e_c padding_10unit_15unit Flex5"
							:class="nValue.data && nValue.data.steps1 && nValue.data.steps2 ? '' : 'fontColorADADAD'"
						>
							{{
					...
					<block v-if="linkageNum == 3">
						<view
							class="flex_row_e_c padding_10unit_15unit Flex5"
							:class="
								nValue.data && nValue.data.steps1 && nValue.data.steps2 && nValue.data.steps3 ? '' : 'fontColorADADAD'
							"
						>
							{{
				...
				<block v-else>
					<block v-if="nValue.data && nValue.data.length > 0">
						<view class="flex_row_e_c padding_10unit_15unit Flex5">
							<view v-for="(i, d) in nValue.data" :key="d">
								{{
									d == 0
										? steps
											? steps.step_1_value
												...
												: i
											: i
										: '-' + (steps ? (steps.step_1_value ? i[steps.step_1_value] : i) : i)
								}}
							</view>
						</view>
					</block>
					<block v-else>
						<view class="flex_row_e_c padding_10unit_15unit Flex5 fontColorADADAD">{{ placherhold }}</view>
					</block>
				</block>
				<view class="Flex1_5 flex_row_c_c"><uniIcon type="forward" :size="fontSize" color="#666"></uniIcon></view>
			</view>
		</QStemplate>
		<pickerCustom2
			ref="inputsRef"
