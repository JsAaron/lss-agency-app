<template>
	<QStemplate :title="title" :titleHide="titleHide" :fontSize="fontSize" :width="width" :titleFlex="titleFlex"
	 ...
	 :layout="layout" :titleLayout="titleLayout" :itlabelStyleemDisabled="itemDisabled" :titleColor="titleColor">
		
		<view v-if="labelText" slot="labelText">
			<text :class="[getLabelLayout]" :style="labelStyle">{{labelText}}</text>
		</view>
		 
		<view class="flex_row_none_c transition_point6s width100 padding_10unit_15unit" :style="
			...
			<view class="flex_row_none_c Flex5">
				<view class="Flex1" v-if="leftIcon">
					<view class="flex_row_c_c width100">
						<uniIcon :type="leftIcon" :size="iconSize" :color="leftIconColor||'#999'" />
					</view>
				</view>
				<view class="Flex5 position_relative">
					...
					:placeholder="placeholder||('请输入' + title)"
					 ...
					 :disabled="disabled" 
					 :style="{'font-size': fontSize + 'rpx', 'opacity': itemDisabled?0:1,'color': inputColor}"
					 :focus="focusBl" 
			...
			<view class="Flex1_5" v-if="tapClear">
				<view class="flex_row_c_c width100" v-if="value" @tap.prevent.stop="inputTap('clear')">
					<uniIcon type="clear" :size="iconSize" color="#999999"></uniIcon>
				</view>
			</view>
			<view class="Flex1_5" v-if="password">
				<view class="flex_row_c_c width100" @tap.prevent.stop="inputTap('password')">
					<uniIcon type="eye" :size="iconSize" :color="passwordBl?'#999999':passwordIconColor||'#33cc33'"></uniIcon>
				</view>
			</view>
			<view class="Flex1_5" v-if="customTapIcon">
				<view class="flex_row_c_c width100" @tap.prevent.stop="inputTap('custom')">
					<uniIcon :type="customTapIcon" :size="iconSize" :color="customTapIconColor||'#999'"></uniIcon>
				</view>
			</view>
			<view class="Flex2 flex_row_c_c" v-if="rightButtonTex">
				<button type="primary" size="mini" class="width100 text_nowrap margin_10unit" :style="'font-size:' + btnSize + 'rpx;' + rightButtonStyle"
				 @tap="inputTap('button')">
					{{rightButtonTex}}
				</button>
			</view>
			<view class="Flex2 flex_row_c_c" v-if="ifCode">
				<button type="primary" size="mini" class="width100 text_nowrap margin_10unit" :disabled="startCodeBl" :style="'font-size:' + btnSize + 'rpx;' + codeButtonStyle"
				 @tap="getCode()">
