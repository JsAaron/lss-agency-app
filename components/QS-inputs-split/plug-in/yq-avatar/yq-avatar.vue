...
export default {
	...
	created() {
		...
		this.imgSrc.imgSrc = this.avatarSrc;
		this.letRotate =
			this.canRotate === false || this.inner === true || this.inner === 'true' || this.canRotate === 'false' ? 0 : 1;
	...
	methods: {
		...
		fSelect() {
			...
			uni.chooseImage({
				...
				success: r => {
					...
					this.imgPath = r.tempFilePaths[0];
		...
		fDrawInit(ini = false) {
			...
			cco.setStrokeStyle('#fff');
			cco.strokeRect(left, top, width, height);

			cco.setFillStyle('#000');
			cco.setGlobalAlpha(0.7);
		...
		fDrawImage(ini = false) {
			...
			cc.save();
			// #endif
			// this.bgImage = '/static/func/ydyj.png'
			cc.setFillStyle('white');
			if (this.bgImage) {
				// #ifdef MP-ALIPAY
		...
		fChooseImg(index = undefined, params = undefined, data = undefined) {
			if (params) {
				...
				expWidth &&
					(this.eW = expWidth.toString().indexOf('upx') >= 0 ? parseInt(expWidth) * this.pxRatio : parseInt(expWidth));
				expHeight &&
					(this.eH =
						expHeight.toString().indexOf('upx') >= 0 ? parseInt(expHeight) * this.pxRatio : parseInt(expHeight));
