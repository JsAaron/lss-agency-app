...
<!--	                <radio color="red" class="radio"></radio>-->
                    <app-radio @click.stop="changeDefault" width="32" height="32" v-model="item.is_default == 1"
                               type="round"></app-radio>
                </view>
                <text class="app-text app-text-color-default" v-if="item.is_default === `0`">设为默认</text>
                <text class="app-text app-text-color-selection" v-else-if="item.is_default === `1`">
                    已设为默认
                </text>
            </view>
        ...
        methods: {
            changeDefault() {
                ...
                self.$request({
                    ...
                    if (info.code === 0) {
                        self.$emit('handleAddress', 'price-sort-default.png')
                    } else {
            ...
            edit() {
                uni.navigateTo({url: `/pages/address/address-edit?id=` + this.item.id + '&is_refund_address=' + this.is_refund_address});
            },
            editStop(){
