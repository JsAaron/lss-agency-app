<template>
	<view class="app-associated-link dir-left-nowrap main-left cross-center" :style="{backgroundColor: background}">
		<app-jump-button style="width: 100%" form :open_type="link.open_type" :url="link.new_link_url" :params="link.params">
			<view :class="[`${className}`]" :style="{backgroundColor: background}">
				<view v-if="styleNum == '1'" class="app-link-title" :style="{textAlign: position}">
					<image class="app-title-icon" v-if="picSwitch" :src="picUrl">
					</image>
					<text :style="{color: color}">{{title}}</text>
				</view>
				<view v-else-if="styleNum == '2'" class="app-link-title main-center cross-center">
                    <image :style="{backgroundColor: styleColor}" class="app-title-line" :src="styleImg.line"></image>
                    <view :style="{color: color}">{{title}}</view>
                    <image :style="{backgroundColor: styleColor}" class="app-title-line" :src="styleImg.line"></image>
                </view>
                <view v-else-if="styleNum == '3'" class="app-link-title main-center cross-center">
                ...
                <view v-else-if="styleNum == '4'" class="app-link-title dir-left-nowrap cross-center">
                    <image :style="{backgroundColor: styleColor}" class="app-title-line div" :src="styleImg.div"></image>
                    <view :style="{color: color}">{{title}}</view>
                </view>
                <view v-else-if="styleNum == '5'" class="app-link-title dir-left-nowrap cross-center">
